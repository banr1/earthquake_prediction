...
if __name__ == '__main__':
    ...
    data_path = '~/kaggle-recruit/input/'
    ...
    data = pd.concat([air_visit, submission], sort=True).copy()
...
def left_merge(data1, data2, on):
    ...
    print('--left_merge end--')
    return result

def diff_of_days(day1, day2):
    #print('--diff_of_days start-- ({} ~ {})'.format(day1, day2))
    days = (parse(day1[:10]) - parse(day2[:10])).days
    #print('--diff_of_days end--')
    return days

def date_add_days(start_date, days):
    #print('--date_add_days start-- (start: {} days: {})'.format(start_date, days))
    end_date = parse(start_date[:10]) + timedelta(days=days)
    end_date = end_date.strftime('%Y-%m-%d')
    #print('--date_add_days end--')
    return end_date

def get_label(end_date, n_day):
    print('--get_label start--')
...
def get_store_visitor_feat(label, key, n_day):
    print('--get_store_visitor_feat start--')
    ...
    print('--get_store_visitor_feat end--')
    return result

def get_store_exp_visitor_feat(label, key, n_day):
    print('--get_store_exp_visitor_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    result['store_exp_mean{}'.format(n_day)] = result['store_exp_mean{}'.format(n_day)]/result['store_exp_weight_sum{}' \
                                                                        .format(n_day)]
    result = left_merge(label, result, on=['store_id']).fillna(0)
    print('--get_store_exp_visitor_feat end--')
    return result

def get_store_week_feat(label, key, n_day):
    print('--get_store_week_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result = data_temp.groupby(['store_id', 'dow'], as_index=False)['visitors'] \
            .agg({'store_dow_min{}'.format(n_day): 'min',
    ...
    print('--get_store_week_feat end--')
    return result

def get_store_week_diff_feat(label, key, n_day):
    print('--get_store_week_diff_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    result = left_merge(label, \
                        result[['store_diff_mean', 'store_diff_std', 'store_diff_max', 'store_diff_min']], on=['store_id']) \
             .fillna(0)
    print('--get_store_week_diff_feat end--')
    return result

def get_store_all_week_feat(label, key, n_day):
    print('--get_store_all_week_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result_temp = data_temp.groupby(['store_id', 'dow'], as_index=False)['visitors'] \
            .agg({'store_dow_mean{}'.format(n_day): 'mean',
                  'store_dow_median{}'.format(n_day): 'median',
    ...
    print('--get_store_all_week_feat end--')
    return result

def get_store_week_exp_feat(label, key, n_day):
    print('--get_store_week_exp_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0], x))
    ...
    for i in [0.9, 0.95, 0.97, 0.98, 0.985, 0.99, 0.999, 0.9999]:
        ...
        result1 = data_temp.groupby(['store_id', 'dow'], as_index=False)['visitors1'] \
                  .agg({'store_dow_exp_mean{}_{}'.format(n_day, i): 'sum'})
        result3 = data_temp.groupby(['store_id', 'dow'], as_index=False)['visitors2'] \
                  .agg({'store_dow_exp_mean2{}_{}'.format(n_day, i): 'sum'})
        result2 = data_temp.groupby(['store_id', 'dow'], as_index=False)['weight'] \
                  .agg({'store_dow_exp_weight_sum{}_{}'.format(n_day, i): 'sum'})
        ...
        result_temp['store_dow_exp_mean{}_{}'.format(n_day, i)] = \
                result_temp['store_dow_exp_mean{}_{}' \
                .format(n_day, i)]/result_temp['store_dow_exp_weight_sum{}_{}'.format(n_day, i)]
        result_temp['store_dow_exp_mean2{}_{}'.format(n_day, i)] = \
                result_temp[ 'store_dow_exp_mean2{}_{}' \
                .format(n_day, i)]/result_temp['store_dow_exp_weight_sum{}_{}'.format(n_day, i)]
        if result is None:
            result = result_temp
        else:
    ...
    result = left_merge(label, result, on=['store_id', 'dow']).fillna(0)
    print('--get_store_week_exp_feat end--')
    return result

def get_store_holiday_feat(label, key, n_day):
    print('--get_store_holiday_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result1 = data_temp.groupby(['store_id', 'holiday_flg'], as_index=False)['visitors'] \
            .agg({'store_holiday_min{}'.format(n_day): 'min',
    ...
    result2 = data_temp.groupby(['store_id', 'holiday_flg2'], as_index=False)['visitors'] \
            .agg({'store_holiday2_min{}'.format(n_day): 'min',
    ...
    print('--get_store_holiday_feat end--')
    return result

def get_genre_visitor_feat(label, key, n_day):
    print('--get_genre_visitor_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    print('--get_genre_visitor_feat end--')
    return result

def get_genre_exp_visitor_feat(label, key, n_day):
    print('--get_genre_exp_visitor_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0], x))
    data_temp['weight'] = data_temp['visit_date'].apply(lambda x: 0.985**x)
    data_temp['visitors'] = data_temp['visitors'] * data_temp['weight']
    result1 = data_temp.groupby(['air_genre_name'], as_index=False)['visitors'] \
            .agg({'genre_exp_mean{}'.format(n_day): 'sum'})
    result2 = data_temp.groupby(['air_genre_name'], as_index=False)['weight'] \
            .agg({'genre_exp_weight_sum{}'.format(n_day): 'sum'})
    result = result1.merge(result2, on=['air_genre_name'], how='left')
    result['genre_exp_mean{}'.format(n_day)] = \
            result['genre_exp_mean{}'.format(n_day)]/result['genre_exp_weight_sum{}'.format(n_day)]
    result = left_merge(label, result, on=['air_genre_name']).fillna(0)
    print('--get_genre_exp_visitor_feat end--')
    return result

def get_genre_week_feat(label, key, n_day):
    print('--get_genre_week_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result = data_temp.groupby(['air_genre_name', 'dow'], as_index=False)['visitors'] \
            .agg({'genre_dow_min{}'.format(n_day): 'min',
    ...
    print('--get_genre_week_feat end--')
    return result

def get_genre_week_exp_feat(label, key, n_day):
    print('--get_genre_week_exp_feat start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0], x))
    data_temp['weight'] = data_temp['visit_date'].apply(lambda x: 0.985**x)
    data_temp['visitors'] = data_temp['visitors'] * data_temp['weight']
    result1 = data_temp.groupby(['air_genre_name', 'dow'], as_index=False)['visitors'] \
              .agg({'genre_dow_exp_mean{}'.format(n_day): 'sum'})
    result2 = data_temp.groupby(['air_genre_name', 'dow'], as_index=False)['weight'] \
    ...
    result['genre_dow_exp_mean{}'.format(n_day)] = \
            result['genre_dow_exp_mean{}'.format(n_day)]/result['genre_dow_exp_weight_sum{}'.format(n_day)]
    result = left_merge(label, result, on=['air_genre_name', 'dow']).fillna(0)
    print('--get_genre_week_exp_feat end--')
    return result

def get_first_last_time(label, key, n_day):
    print('--get_first_last_time start--')
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    result = left_merge(label, result, on=['store_id']).fillna(0)
    print('--get_first_last_time end--')
    return result

def get_reserve_feat(label,key):
    print('--get_reserve_feat start--')
    ...
    air_reserve_temp['diff_time'] = \
            (pd.to_datetime(air_reserve['visit_datetime']) - pd.to_datetime(air_reserve['reserve_datetime'])).dt.days
    air_reserve_temp = air_reserve_temp.merge(air_store, on='store_id')
    air_result = air_reserve_temp.groupby(['store_id', 'visit_date'])['reserve_visitors'] \
            .agg({'air_reserve_visitors': 'sum',
                  'air_reserve_count': 'count'})
    air_store_diff_time_mean = air_reserve_temp.groupby(['store_id', 'visit_date'])['diff_time'] \
            .agg({'air_store_diff_time_mean': 'mean'})
    air_diff_time_mean = air_reserve_temp.groupby(['visit_date'])['diff_time'] \
            .agg({'air_diff_time_mean': 'mean'})
    air_result = air_result.unstack().fillna(0).stack()
    air_date_result = air_reserve_temp.groupby(['visit_date'])['reserve_visitors'] \
            .agg({'air_date_visitors': 'sum',
                  'air_date_count': 'count'})
    hpg_reserve_temp = \
            hpg_reserve[(hpg_reserve.visit_date >= key[0]) & \
            (hpg_reserve.visit_date < label_end_date) & \
            (hpg_reserve.reserve_date < key[0])].copy()
    hpg_reserve_temp['diff_time'] = \
            (pd.to_datetime(hpg_reserve['visit_datetime']) - pd.to_datetime(hpg_reserve['reserve_datetime'])).dt.days
    hpg_result = hpg_reserve_temp.groupby(['store_id', 'visit_date'])['reserve_visitors'] \
            .agg({'hpg_reserve_visitors': 'sum',
                  'hpg_reserve_count': 'count'})
    hpg_result = hpg_result.unstack().fillna(0).stack()
    hpg_date_result = hpg_reserve_temp.groupby(['visit_date'])['reserve_visitors'] \
            .agg({'hpg_date_visitors': 'sum',
            'hpg_date_count': 'count'})
    hpg_store_diff_time_mean = hpg_reserve_temp.groupby(['store_id', 'visit_date'])['diff_time'] \
            .agg({'hpg_store_diff_time_mean': 'mean'})
    ...
    print('--get_reserve_feat end--')
    return result

def second_feat(result):
    print('--second_feat start--')
...
def make_feats(end_date,n_day):
    ...
    print('make feature...')
    result = [label]
    result.append(get_store_visitor_feat(label, key, 1000))
    result.append(get_store_visitor_feat(label, key, 56))
    result.append(get_store_visitor_feat(label, key, 28))
    result.append(get_store_visitor_feat(label, key, 14))
    result.append(get_store_exp_visitor_feat(label, key, 1000))
    ...
    result.append(get_store_all_week_feat(label, key, 1000))
    result.append(get_store_week_exp_feat(label, key, 1000))
    result.append(get_store_holiday_feat(label, key, 1000))
    result.append(get_genre_visitor_feat(label, key, 1000))
    result.append(get_genre_visitor_feat(label, key, 56))
    result.append(get_genre_visitor_feat(label, key, 28))
    result.append(get_genre_exp_visitor_feat(label, key, 1000))
...
def main():
    ...
    predictors = [f for f in test_feat.columns \
                   if f not in (['id', 'store_id', 'visit_date', 'end_date', 'air_area_name', 'visitors', 'month'])]
