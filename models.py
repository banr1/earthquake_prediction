...
class SimpleRNNmodel(Basemodel):
    def build_model(self, float_data, lookback, batch_size, optimizer, loss, stateful, target_length,
        ...
        x = SimpleRNN(num_layers[0], activation='relu', stateful=stateful,
...
class GRUmodel(Basemodel):
    def build_model(self, float_data, lookback, batch_size, optimizer, loss, stateful, target_length,
        ...
        if stateful:
            ...
            input = Input(shape=(None, float_data.shape[-1]))
        x = GRU(num_layers[0], activation='tanh', stateful=stateful,
                dropout=dropouts[0], recurrent_dropout=recurrent_dropouts[0])(input)
        output = Dense(target_length, activation='relu')(x)
        model = Model(input, output)
        model.compile(optimizer=optimizer, loss=loss)
...
class LSTMmodel(Basemodel):
    def build_model(self, float_data, lookback, batch_size, optimizer, loss, stateful, target_length,
        ...
        if stateful:
            ...
            input = Input(shape=(None, float_data.shape[-1]))
        x = LSTM(num_layers[0], activation='tanh', stateful=stateful,
                 dropout=dropouts[0], recurrent_dropout=recurrent_dropouts[0])(input)
        output = Dense(target_length, activation='relu')(x)
        model = Model(input, output)
        model.compile(optimizer=optimizer, loss=loss)
...
class StackedGRUmodel(Basemodel):
    def build_model(self, float_data, lookback, batch_size, optimizer, loss, stateful, target_length,
        ...
        if stateful:
            ...
            input = Input(shape=(None, float_data.shape[-1]))
        x = GRU(num_layers[0], activation='tanh', stateful=stateful, return_state=True,
                dropout=dropouts[0], recurrent_dropout=recurrent_dropouts[0])(input)
        x = GRU(num_layers[1], activation='relu', stateful=stateful, return_state=False,
...
class StackedLSTMmodel(Basemodel):
    def build_model(self, float_data, lookback, batch_size, optimizer, loss, stateful, target_length,
        ...
        if stateful:
            ...
            input = Input(shape=(None, float_data.shape[-1]))
        x = LSTM(num_layers[0], activation='tanh', stateful=stateful, return_state=True,
                dropout=dropouts[0], recurrent_dropout=recurrent_dropouts[0])(input)
        x = LSTM(num_layers[1], activation='relu', stateful=stateful, return_state=False,
                dropout=dropouts[1], recurrent_dropout=recurrent_dropouts[1])(input)
        output = Dense(target_length, activation='relu')(x)
        model = Model(input, output)
        model.compile(optimizer=optimizer, loss=loss)
